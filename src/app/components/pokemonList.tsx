...
export default function PokemonList() {
    ...
    const [seletedTypeFilter, setSelectedTypeFilter] = useState<string>('All Type');
    ...
    const handleCategory = (url: string, name: string) : void  => {
        fetchData(url);
        setIsTypeFilter(!isTypeFilter);
    ...
    return (
        <>
            <main className={'bg-amber-50'}>
                <section className={'bg-[#EFF3F6]'}>
                    <div className={'container'}>
                        <div
                            className={'flex flex-col gap-4 lg:gap-0 lg:flex-row justify-between items-center py-28 lg:py-20'}>
                            <h2 className={'w-full text-center lg:text-left lg:max-w-[250px] leading-tight text-[32px] font-bold text-[#2F3133]'}>Choose
                                your Pokémon</h2>
                ...
                <section className={'bg-white'}>
                    <div className={'container'}>
                        <div className={'flex flex-col py-12 lg:py-0 lg:flex-row'}>
                            <div className={'w-full lg:w-3/12'}>
                                <div onClick={() => handleIsTypeFilter()}
                                     className={'flex gap-2 items-center lg:px-0 mt-6 lg:mt-0 lg:py-[47px] border lg:border-none border-[#A0AFBA] md:cursor-not-allowed lg:border-transparent rounded-full py-4 px-6'}>
                                    <Image
                                        ...
                                        priority
                                    />
                                    <h3 className={'text-sm lg:text-lg font-semibold text-[#3E75C3]'}>Selected Type
                                        : <span className={'capitalize'}>{seletedTypeFilter}</span></h3>
                                </div>
                                <div
                                    className={isTypeFilter ? 'flex flex-col sm:border-r sm:border-r-slate-200 py-6 md:py-0 sm:pr-24 fixed md:static bg-white h-screen z-10 pl-[1.5em] pr-[1.5em] md:pl-0  w-full left-0 top-0 overflow-scroll' : 'hidden md:flex md:flex-col sm:border-r sm:border-r-slate-200 sm:pr-24'}>
                                    <button className={'ml-auto md:hidden'} onClick={() => setIsTypeFilter(false)}>
                                    ...
                                    {pokemonType?.map((data: any, index: any): any => {
                                            return (
                                                <div key={index} onClick={() => handleCategory(data?.url, data?.name)}
                                                     className={seletedTypeFilter == data?.name ? 'text-[#3E75C3] capitalize cursor-pointer text-sm font-semibold py-2.5 hover:text-[#3E75C3]' : 'capitalize cursor-pointer text-sm font-semibold text-[#ACB9C1] py-2.5 hover:text-[#3E75C3]'}>{data?.name}</div>
                                            )
                                        }
                                    )}
                                </div>
                            ...
                            <div className={'w-full lg:w-9/12 lg:pl-[60px] xl:pl-[100px] 2xl:pl-[116px]'}>
                                <div className={'flex flex-row justify-between'}>
                                    ...
                                    <div onClick={() => handleSortBy()}
                                         className={'flex gap-4 items-center py-6 md:py-[47px] cursor-pointer'}>
                                        <svg width="10" height="16" viewBox="0 0 10 16" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                            <path d="M4.92308 0L9.84615 6.76923H0L4.92308 0Z" fill="#3E75C3"
                                ...
                                {sortedPokemon.length > 0 ? (
                                    <div className={'grid  grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-8 lg:gap-6 xl:gap-8 pb-16'}>
                                        {
                                                ...
                                                return (
                                                    <>
                                                        <div className={'bg-white p-6 rounded-xl card-shadow'}
                                                            ...
                                                            <ImagePokemon pokemonId={pokemonId}
                                                                          pokemonName={data.name ? data?.name : data?.pokemon?.name}/>
                                                            {/*<img*/}
                                    ...
                                    <div
                                        className={'bg-white card-shadow flex rounded-xl mb-12 justify-center items-center py-32'}>
                                        <p className={'text-[#A0AFBA]'}>Data Tidak Ada</p>
                                    </div>
                                )}
            ...
            {/*  Dialog  */}
            <div id={'dialog'}
                 className={isDialogOpen ? 'flex justify-center items-end md:items-center fade-in' : 'hidden'}>
